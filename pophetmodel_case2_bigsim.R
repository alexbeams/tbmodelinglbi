...
gettheta <- function(f,w,fH=0.9,pH=0.1){
	...
	zeta <- (-2*N*f - Ieq - Leq + 2*N)/((theta*w - theta - w)*(-2*N*f + Ieq + Leq))
 
	# What is the baseline transmission rate, beta, in the perfect mixing case?
	beta0 <- -2*(Ieq*gam - Leq*sigma)/(p*(2*N*f + Ieq + Leq - 2*N)*Ieq*(theta + 1)) 
 
	# If betaij = w * beta0, so that 0 < w < 1 models assortative mixing, what is betaii?
	betaii <- -2*(-Ieq*w*(Ieq*gam - Leq*sigma)*theta/(2*(2*N*f + Ieq + Leq - 2*N)*(theta + 1)) - Leq*w*(Ieq*gam - Leq*sigma)*theta/(2*(2*N*f + Ieq + Leq - 2*N)*(theta + 1)) + (-N*f + N)*w*(Ieq*gam - Leq*sigma)*theta/((2*N*f + Ieq + Leq - 2*N)*(theta + 1)) + Ieq*gam/2 - Leq*sigma/2)/(p*(f*N - N + 1/2*Leq + 1/2*Ieq)*Ieq) 
...
#
# # Simulate a tree :
# source('pophet_casex_bigsim_gettrees.R')
#} 
#
#
